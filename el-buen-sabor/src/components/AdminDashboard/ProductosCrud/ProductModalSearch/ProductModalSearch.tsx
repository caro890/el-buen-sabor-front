...
export const ProductModalSearch : FC<IPropsProductModalSearch> = ({open, handleClose, options}) => {
  ...
  const handleCheckButtonChange = (item: ArticuloInsumo, checked: boolean) => {
    var aux: ArticuloInsumo[] = ingredientes.slice();   //copio el estado de insumo
    ...
    dispatch(setIngredientes(aux));    //seteo el estado detalle con el array auxiliar
  };
  ...
  return (
    <Modal show={open} onHide={handleClose} centered size="lg">
        ...
        <Modal.Footer>
          ...
          <Button className="btnAdd" type="button" onClick={handleClose}>AGREGAR</Button> {/*Boton para agregar los detalles elegidos*/}
        </Modal.Footer>
      </Modal>
  )
}
